...
n_periodos = 10  # Número de períodos futuros a projetar
TAXA_DE_ANALISE = 0.65
...
for i, col in enumerate(value_cols):
    plt.plot(df.index, df[col].values, label=f'Histórico Real ({col})', alpha=0.7, linewidth=2)
    plt.plot(datas_futuras, futuro[:, i], label=f'Projeção Futura ({col})', linestyle=':', marker='o', alpha=0.7)

plt.title('Análise Random Forest - Histórico Completo e Projeção Futura')
plt.xlabel('Tempo')
plt.ylabel('Valores')
plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
plt.grid(True)
plt.tight_layout()
plt.savefig('previsao_rf_completa.png', bbox_inches='tight', dpi=300)
plt.close()
...
print("📁 Gráfico salvo como previsao_rf_completa.png")
print("\n📊 Erro MSE por sensor:")
